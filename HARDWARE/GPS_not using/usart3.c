...
void USART3_IRQHandler(void)                	
{
	...
	{
		Res[counter] =USART_ReceiveData(USART3);//(USART1->DR);	//读取接收到的数据
		counter++;
		if(Res[0]!='$')
			USART3_Clear_Buff();
		if(counter>1&&Res[counter-1]=='\n'&&Res[counter-2]=='\r'){
			USART3_Process();
			USART3_Clear_Buff();
		}
  } 
...
void process_GPGGA(void){
	...
	for(j=0;j<count-1;j++){
		...
		switch(j){
			case 0:	temp = atof(temp_arr);	GPGGA_Dat.Hour = (u32)temp/10000;	GPGGA_Dat.Minute = ((u32)temp-10000*GPGGA_Dat.Hour)/100; GPGGA_Dat.Second = temp-10000*GPGGA_Dat.Hour-100*GPGGA_Dat.Minute;	GPGGA_Dat.Hour+=8;		break;
			case 1:	temp = atof(temp_arr);	temp_h = temp/100;	temp_m = (temp/100-(int)(temp/100))/0.6f;	GPGGA_Dat.latitude = (float)temp_h+temp_m;				break;
			case 2: GPGGA_Dat.NorS = temp_arr[0];										break;
			case 3: temp = atof(temp_arr);	temp_h = temp/100;	temp_m = (temp/100-(int)(temp/100))/0.6f;	GPGGA_Dat.longittude = (float)temp_h+temp_m;				break;
			...
			case 7: GPGGA_Dat.accurate_horizontal = atof(temp_arr); break;
			case 8:	GPGGA_Dat.altitute = atof(temp_arr);						break; 
		}
		for(;k>0;k--){
			temp_arr[k] = 0;
		}
...
void process_GPGSA(void){
	...
	for(i=0;i<counter+1;i++){
		...
		switch(j){
			...
			case 14: GPGSA_Dat.PDOP_accuracy = atof(temp_arr);					break;
			case 15: GPGSA_Dat.HDOP_accuracy = atof(temp_arr);					break;
			case 16: GPGSA_Dat.VDOP_accuracy = atof(temp_arr);					break;
...
void process_GPRMC(void){
	...
	for(j=0;j<count-1;j++){
		...
		//set to the structure
		switch(j){
			case 0:	temp = atof(temp_arr);	GPRMC_Dat.Hour = (u32)temp/10000;	GPRMC_Dat.Minute = ((u32)temp-10000*GPRMC_Dat.Hour)/100; GPRMC_Dat.Second = temp-10000*GPRMC_Dat.Hour-100*GPRMC_Dat.Minute;	GPRMC_Dat.Hour+=8;		break;
			case 1:	GPRMC_Dat.locate_state = temp_arr[0];						break;
			case 2:	temp = atof(temp_arr);	temp_h = temp/100;	temp_m = (temp/100-(int)(temp/100))/0.6f;	GPRMC_Dat.latitude = (float)temp_h+temp_m;				break;
			case 3: GPRMC_Dat.NorS = temp_arr[0];										break;
			case 4: temp = atof(temp_arr);	temp_h = temp/100;	temp_m = (temp/100-(int)(temp/100))/0.6f;	GPRMC_Dat.longittude = (float)temp_h+temp_m;				break;
			case 5: GPRMC_Dat.EorW = temp_arr[0];										break;
			case 6:	GPRMC_Dat.ground_speed = atof(temp_arr);				break;
			case 7:	GPRMC_Dat.ground_dir = atof(temp_arr);					break;
			case 8:	temp = atoi(temp_arr);	GPRMC_Dat.day = (u32)temp/10000;	GPRMC_Dat.month = ((u32)temp-10000*GPRMC_Dat.day)/100; GPRMC_Dat.year = temp-10000*GPRMC_Dat.day-100*GPRMC_Dat.month;	break;
			case 9:	GPRMC_Dat.mag_var = atof(temp_arr);							break;
			case 10:GPRMC_Dat.Declination = temp_arr[0];						break;
			case 11:GPRMC_Dat.Mode_ini = atoi(temp_arr);						break;
...
// if it's the same word,return 0,else return 1
